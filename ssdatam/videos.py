...
def display_video_clip(
    ...
    img = ax.imshow(
        video_tensor[:, :, 0], cmap="gray", animated=True, vmin=vmin, vmax=vmax
    )
    ax.axis("off")
...
def pick_key_frames(
    ...
    segments_frame_end: np.ndarray | list[int],
    frames_per_segment: int | None = None,
    distance_between_key_frames: int | None = None,
) -> np.ndarray:
    ...
    if (
        distance_between_key_frames is None
        or isinstance(distance_between_key_frames, int)
    ) == False:
        raise ValueError(
            "distance_between_key_frames must be either None or an integer"
        )
    if (distance_between_key_frames is None) and (frames_per_segment is None):
        raise ValueError(
            "at least one of distance_between_key_frames or frames_per_segment must be an integer"
        )
    ...
    for start_frame, end_frame in zip(segments_frame_start, segments_frame_end):
        if isinstance(frames_per_segment, int) and distance_between_key_frames is None:
            new_frames = np.linspace(start_frame, end_frame, frames_per_segment + 2)[
                1:-1
            ]
        elif (
            ...
            distance_between_key_frames, int
        ):
            new_frames_a = np.linspace(start_frame, end_frame, frames_per_segment + 2)[
                1:-1
            ]
            new_frames_b = np.arange(
...
def find_edges(
    x: np.ndarray, max_transition_frames: int, limit: float | int, revert: bool = False
) -> int:
...
def select_peaks(
    peaks: np.ndarray, priority: np.ndarray, distance: float | int
) -> np.ndarray:
...
def find_margin(
    data: np.ndarray, limit: float | int = 0, axis: int = 0, revert: bool = False
) -> int:
...
class Video:
    ...
    def __init__(
        ...
        self.trial = trial
        self.data = np.load(
            os.path.join(recording_folder, "data", "videos", trial + ".npy")
        )
    ...
    def find_peaks(
        ...
        relative_threshold: bool | None = None,
        min_thresh: float | int | None = None,
        threshold_outliers: float | int | None = None,
    ) -> None:
    ...
    def define_segments(
        self, frame_start: list[int] | None = None, frame_end: list[int] | None = None
    ) -> dict:
        ...
        if set_based_on_peaks:
            ...
            if not hasattr(self, "peaks"):
                raise AttributeError(
                    "The peaks attribute is missing. Run find_peaks before"
                )

            # initialize the segments dictionary
            self.n_segments = self.n_peaks + 1
            segments = {
                "frame_start": np.empty(self.n_segments, dtype=int),
        ...
        for idx, (ki, kf) in enumerate(
            zip(segments["frame_start"], segments["frame_end"])
        ):
    ...
    def split_long_segments_by_label(self, label: str) -> dict | None:
        ...
        if not hasattr(self, "segments"):
            raise AttributeError(
                "The segments attribute is missing. Run define_segments before"
            )
        ...
        if segm_dur != None:
            ...
            idx_segm = (n_times > 1) & (
                np.abs(self.segments["duration"] - n_times * segm_dur) < 4
            )
            if np.any(idx_segm):
                add_segment_start = []
                add_segment_end = []
                ...
                for k in idx_segm:
                    n_frames_segm = np.round(
                        self.segments["duration"][k] / n_times[k]
                    ).astype(int)
                    segments_new["frame_end"][k] = (
                        self.segments["frame_start"][k] + n_frames_segm
                    )
                    for j in np.arange(1, n_times[k], 1):
                        add_segment_start.append(
                            self.segments["frame_start"][k] + j * n_frames_segm
                        )
                        add_segment_end.append(
                            self.segments["frame_start"][k] + (j + 1) * n_frames_segm
                        )
                    add_segment_end[-1] = self.segments["frame_end"][k]
                segments_new["frame_start"] = np.sort(
                    np.concatenate((segments_new["frame_start"], add_segment_start))
                )
                segments_new["frame_end"] = np.sort(
                    np.concatenate((segments_new["frame_end"], add_segment_end))
                )
        ...
        for idx, (ki, kf) in enumerate(
            zip(segments_new["frame_start"], segments_new["frame_end"])
        ):
        ...
        for i in range(self.n_segments):
            ...
            if i == 0:
                for key in segm_i.properties.keys():
                    self.segments[key] = np.empty(
                        self.n_segments, dtype=type(segm_i.properties[key])
                    )
            for key in segm_i.properties.keys():
    ...
    def is_gaussiandot(self) -> tuple[bool, np.ndarray]:
        ...
        max_segments_not_maching = (
            2  # maximun number of segments not matching the requierments
        )
        ...
        good_duration = np.logical_and(
            self.segments["duration"] >= (duration - duration_tolerance),
            self.segments["duration"] <= (duration + duration_tolerance),
        )
        ...
        has_background = (
            self.segments["background_proportion"] >= thresh_background_proportion
        )
        ...
        if (n_segments_diff <= n_segments_tolerance) & (
            n_segments_bad_properties <= max_segments_not_maching
        ):
            is_gaussian = True
        else:
            is_gaussian = False
    ...
    def is_naturalimages(self) -> tuple[bool, np.ndarray]:
        ...
        max_segments_not_maching = (
            4  # maximun number of segments not matching the requierments
        )
        ...
        if any(self.segments["is_uniform"]):
            ...
            good_duration_black = np.logical_and(
                dur_black >= (duration_black[0] - duration_tolerance),
                dur_black <= (duration_black[1] + duration_tolerance),
            )
            good_duration_immage = np.logical_and(
                dur_image >= (duration_image - duration_tolerance),
                dur_image <= (duration_image + duration_tolerance),
            )
    ...
    def is_randomdots(self) -> tuple[bool, np.ndarray]:
        ...
        max_segments_not_maching = (
            1  # maximun number of segments not matching the requierments
        )
    ...
    def is_naturalvideo(
        self, lim_segments: int = 3, min_var_duration: float | int = 1
    ...
    def plot_intensity_hist(self, segment) -> tuple[plt.Figure, plt.Axes]:
        ...
        """
        ki = (
            self.segments["frame_start"][segment]
            + self.segments["transition_start"][segment]
        )
        kf = (
            self.segments["frame_end"][segment]
            - self.segments["transition_end"][segment]
        )
        img = self.data[:, :, ki:kf]
        fig, ax = plt.subplots(1, 1)
        ax.hist(img.flatten(), range=(0, 255), bins=255)
    ...
    def plot_frames(
        self, frames: np.ndarray | list[int], ncol: int = 5
        ...
        axs = np.array(
            ax, ndmin=1
        ).ravel()  # robust handling of single Axes / 1D / 2D arrays
    ...
    def convert_to_dict(
        ...
        segments_fields: list[str] | str | None,
        ...
        # segments
        if not (
            (segments_fields == None)
            or (segments_fields == "all")
            or isinstance(segments_fields, list)
        ):
            raise ValueError(
                "segments_fields can be ''all'', None, or a list of strings"
            )
        ...
        if convert_duplicates and hasattr(self, "duplicates"):
            ...
            for rec in self.duplicates.keys():
                ...
                meta_dict["duplicates"][rec]["trials"] = list(
                    self.duplicates[rec]["trials"]
                )
    ...
    def save_metadata(
        self,
        folder_metadata: str | Path,
        main_fields: list[str] | None = None,
        segments_fields: list[str] | str | None = None,
        save_duplicates: bool | None = None,
        ...
        meta_video = self.convert_to_dict(
            main_fields, segments_fields, convert_duplicates=save_duplicates
        )
...
class VideoData(Video):
    ...
    def __init__(
        ...
        trial: str | None = None,
        sampling_freq: float | int | None = None,
        label: str | None = None,
        ID: str | None = None,
    ):
...
class VideoID(Video):
    ...
    def __init__(
        self, data_folder: str | Path, videos_metadata_folder: str | Path, video_id: str
    ):
        ...
        self.trial = str(
            random.choice(list(metavideo["duplicates"][self.recording]["trials"]))
        )

        # load an exemplar video
        recording_folder = os.path.join(data_folder, self.recording)
        self.data = np.load(
            os.path.join(recording_folder, "data", "videos", self.trial + ".npy")
        )
...
class VideoSegment:
    ...
    def __init__(
        ...
        if not isinstance(video, (Video, np.ndarray)):
            raise ValueError(
                "video must be either a numpy array or an instance of Video"
            )
        ...
        if isinstance(video, Video):
            ...
            if segment_index == None:
                raise ValueError(
                    "An index indicating the segment to take must be provided"
                )
            self.parentvideo["segment_index"] = segment_index
            self.parentvideo["frame_start"] = video.segments["frame_start"][
                segment_index
            ]
            ...
            data = video.data[
                :, :, self.parentvideo["frame_start"] : self.parentvideo["frame_end"]
            ]
        else:
            data = video
    ...
    def is_static(
        self,
        limit: float | int = thresh_no_change,
        max_transition_frames: int = max_transition_frames,
    ):
    ...
    def is_uniform(
        self,
        thresh_background_proportion: float = 0.8,
        thresh_intensity_range: float | int = thresh_intensity_range,
    ) -> None:
        ...
        self.properties["is_uniform"] = (
            (self.properties["background_proportion"] >= thresh_background_proportion)
    ...
    def label_from_parentvideo(
        self, thresh_intensity_range: float | int = thresh_intensity_range
    ) -> None:
    ...
    def convert_to_dict(
        ...
        main_fields: list[str],
        convert_duplicates: bool = True,
        convert_parentvideo: bool = False,
    ) -> dict:
        ...
        for ff in main_fields:
            if hasattr(self, ff):
                meta_dict[ff] = getattr(self, ff)
        meta_dict = {
            k: int(v) if isinstance(v, np.integer) else v for k, v in meta_dict.items()
        }
        ...
        if convert_duplicates:
            ...
            for videoid in self.duplicates.keys():
                ...
                meta_dict["duplicates"][videoid]["segment_index"] = list(
                    self.duplicates[videoid]["segment_index"]
                )
    ...
    def save_metadata(
        ...
        folder_metadata: str | Path,
        file_name: str | None = None,
        main_fields: list[str] = ["ID", "label", "sampling_freq", "valid_frames"],
        convert_parentvideo: bool = False,
        convert_duplicates: bool = True,
    ):
        ...
        if file_name is None:
            if self.label and self.ID:
                ...
                raise ValueError(
                    "No label and ID found, please provide explicitely a file name"
                )
...
class VideoSegmentID(VideoSegment):
    ...
    def __init__(
        ...
        segment_index = random.choice(
            list(metasegment["duplicates"][videoID]["segment_index"])
        )
