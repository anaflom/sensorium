...
class Neurons:
    ...
    def __init__(self, folder_metadata: str | Path, recording: str) -> None:
        ...
        try:
            ...
            self.coord_xyz = (
                df_neurons[["coord_x", "coord_y", "coord_z"]].copy().to_numpy()
            )
            self.IDs = df_neurons["ID"].copy().to_numpy()
            self.stats_activity = {
                k: df_neurons[k].copy().to_numpy()
                for k in [
                    "mean_activation",
                    "std_activation",
                    "median_activation",
...
class NeuronsData(Neurons):
    ...
    def __init__(
        self,
        coord_xyz: np.ndarray | None,
        IDs: np.ndarray | None,
        stats_activity: dict[str, np.ndarray] | None = None,
    ) -> None:
