...
def same_segments_edges(
    video_i: Video, video_j: Video, frames_tolerance: int = 2
) -> bool:
    ...
    if n_segments_i != n_segments_j:
        ...
        for k in range(n_segments_i):
            same_start = (
                np.abs(
                    video_i.segments["frame_start"][k]
                    - video_j.segments["frame_start"][k]
                )
                <= frames_tolerance
            )
            same_end = (
                np.abs(
                    video_i.segments["frame_end"][k] - video_j.segments["frame_end"][k]
                )
                <= frames_tolerance
            )
...
def compute_dissimilarity_video_list(
    videos: list[Video],
    ...
    for i, video_i in enumerate(
        tqdm(videos, total=len(videos), desc="Computing dissimilarity", disable=False)
    ):
        ...
        for j in np.arange(i, n_videos):
            ...
            if check_edges_first:
                do_comparison = same_segments_edges(
                    video_i, video_j, frames_tolerance=frames_tolerance
                )
            else:
                do_comparison = True
...
def find_equal_sets(
    mask: np.ndarray, elements_names: list[str | int] | None = None
) -> list[set[str | int]]:
...
def create_table_all_video_ids(
    folder_globalmetavideos: str | Path, label: str | None = None
) -> pd.DataFrame:
...
def compare_with_idvideos(
    label: str,
    list_distinct_videos: list[set[str]],
    folder_videos: str | Path,
    folder_metavideos: str | Path,
    folder_globalmetavideos: str | Path,
    ...
    list_new_ids = []
    for i, duplicate_trials in enumerate(
        tqdm(
            list_distinct_videos,
            total=len(list_distinct_videos),
            desc="Comparing with existing ID videos",
            disable=False,
        )
    ):
        ...
        for j, video_id in enumerate(same_label_ids):
            ...
            if (video_i.recording in video_j_id.duplicates) and (
                video_i.trial
                in video_j_id.duplicates[video_i.recording].get("trials", [])
            ):
        ...
        if np.sum(equal_to) == 0:
            ...
            if not already_included_flags[j_match]:
                ...
                video_j_id.save_metadata(
                    folder_globalmetavideos, metadata_for="videoID"
                )

        else:
            raise Exception(
                f"Something went wrong, group {i} found identical to two existing unique videos"
            )
